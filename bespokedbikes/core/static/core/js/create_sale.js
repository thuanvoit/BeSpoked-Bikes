...
function getToday() {
  ...
  if (document.getElementById("sales_date").value) {
    return document.getElementById("sales_date").value;
  }
  return getTodayDate();
}
...
async function handleSaleDateChange() {
  ...
  const data = await getProductDetails(selectedValue);
  const salerdata = await fetchData(`../api/saler-details/`);
  populateSalerByDate(salerdata);
  const customerdata = await fetchData(`../api/customer-details/`);
